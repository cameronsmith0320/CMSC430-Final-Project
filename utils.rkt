...
(define (eval-llvm llvm-str)
  ...
  ;(when (not recent-header)
  ;      (set! recent-header #t)
  ;      ;(system (string-append "clang++ header.cpp " " -I " gc-include-path " -S -emit-llvm -o header.ll"))
  ;      (system (string-append "clang++ header.cpp " " -S -emit-llvm -o header.ll")))
  ;(define header-str (read-string 299999 (open-input-file "header.ll" #:mode 'text)))
  ;(define llvm (string-append header-str "\n\n;;;;;;\n\n" llvm-str))
  ;(define combined-port (open-output-file "combined.ll" #:exists 'replace))
  ;(display llvm combined-port)
  ;(close-output-port combined-port)
  ;;(system (string-append "clang++ combined.ll " libgc-path " -I " gc-include-path " -lpthread -o bin.exe"))
  ;(system (string-append "clang++ combined.ll " " -o bin.exe"))
  ;(match-define `(,out-port ,in-port ,id ,err-port ,callback) (process "bin.exe"))
  ;(define starttime (current-milliseconds))
  (let loop ()
    ;(sleep 1)
...
(define (test-full-compile full-compile prog)
  (define llvm (full-compile prog))
  (define val0 (eval-llvm llvm))
  ...
  (if (equal? val val0)
      #t
      (begin
        (display (format "Test-final-compile: two different values (~a and ~a) before and after compilation.\n"
                         val val0))
        #f)))
