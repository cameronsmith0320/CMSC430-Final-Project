...
u64 prim_make_45vector(u64 lenv, u64 iv)
{
    ...
    if (DECODE_INT(lenv) < 0)
        fatal_err("first argument to make-vector must be non-negative");
    const u64 l = DECODE_INT(lenv);
    u64* vec = (u64*)alloc((l + 2) * sizeof(u64));
...
u64 prim_vector_45ref(u64 v, u64 i)
{
    ...
    if (DECODE_INT(i) < 0)
        fatal_err("second argument to vector-ref must be non-negative");
    u64 l = ((u64*)DECODE_OTHER(v))[1];
    if (DECODE_INT(i) >= l)
        fatal_err("second argument to vector-ref must be less than vector length");
...
u64 prim_vector_45set_33(u64 a, u64 i, u64 v)
{
    ...
    if (DECODE_INT(i) < 0)
        fatal_err("second argument to vector-set! must be non-negative");
    if ((((u64*)DECODE_OTHER(a))[0]&7) != VECTOR_OTHERTAG)
        fatal_err("vector-set! not given a properly formed vector");
    u64 l = ((u64*)DECODE_OTHER(a))[1];
    if (DECODE_INT(i) >= l)
        fatal_err("second argument to vector-set! must be less than vector length");
...
u64 prim__47(u64 a, u64 b) // /
{
    ...
    if (DECODE_INT(b) == 0)
        fatal_err("(prim / a b); division by 0");
    return ENCODE_INT(DECODE_INT(a) / DECODE_INT(b));
}
